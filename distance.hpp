...
If v2 has a greater size, the symbols are discarded (but not added to increase the ser.*/
...
int  ivec_distance_levenshtein(ivec v1, ivec v2, int cost_ins,
	int cost_del, int cost_sub);
...
/* Return the energy of the vector of differences. Vectors should have the same size. */
...
/* compute the matrix of distance associated with a set of vectors
Note that the vectors are assumed to be stored in rows          */
...
/* compute the distance between two sparse vectors */
