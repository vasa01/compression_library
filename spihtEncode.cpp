...
void SPIHT_Encoder::initialize( BitOutputStream* bout) {
	...
	//		//cout << "x=" << x << " y=" << y << endl;
	//		//mno�ina index�(sou�adnic) ko�en�.Spadaj� sem koeficienty z nejvy��� �rovn� rozkladu, p��padn� t� DC koeficient
	//		lip.push_back(PixItem(x, y));
	//		if(x >= (subIm[0].w() / (2)) || y >= (subIm[0].h() / (2))){//if ((x % 2 != 0) || (y % 2 != 0)) {
	//			//cout << "push" << endl;
	//			//pouze ty prvky z lip, kter� maj� p��m� potomky, jako prvky typu A
