...
static inline pmat pmat_new(int W, int H) {
	return ((pmat)Mat_new(vec, W, H));
}
static inline void pmat_delete(pmat m) {
	Mat_delete((Mat)m);
}
...
static inline pvec pvec_new(int N) {
	return ((pvec)Vec_new(vec, N));
}
static inline void pvec_delete(pvec v) {
	Vec_delete((Vec)v);
}
