...
/* scale a complex by a real number */
...
/* return the argument of a complex in ]-pi;pi] */
static inline double cang(cplx a) {
	double theta;
...
#ifdef ARCH_HAS_SSE
static inline void __cplx_swap_sse(cplx * a, cplx * b) {
	asm volatile ("movq  (%0),%%xmm0\n"
		"movq  (%1),%%xmm1\n"
		"movq  %%xmm0,(%1)\n"
		"movq  %%xmm1,(%0)\n"::"r" (a), "r" (b) : "memory");
}
#define cplx_swap(a, b) __cplx_swap_sse(&a, &b)
#else
static inline void __cplx_swap(cplx * a, cplx * b) {
	double t0, t1;
	t0 = a->r;
	t1 = a->i;
	a->r = b->r;
